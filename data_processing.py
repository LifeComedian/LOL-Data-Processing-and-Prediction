...
# -*- coding: utf-8 -*-
"""
Make per-minute CSVs from Riot match timeline JSONs.

Supports:
- final win label per participant (win: 1/0, -1 if unknown)
- cumulative deaths per participant (deaths)
- total_gold (cumulative)
- champion info via either:
  (A) match details JSONs dir [details_dir], or
  (B) overview CSV [overview_csv] containing rows with columns:
      match_id, participant_puuid, champion_id, champion_name
You can provide both; overview_csv takes precedence when both contain values.

Usage:
    python make_minute_csvs.py /path/to/timeline_json_dir /path/to/output_root [details_dir] [overview_csv]
    python data_processing.py C:/Users/gigggjjffd/Desktop/work/school_stuffs/homeworks/STATS302 C:/Users/gigggjjffd/Desktop/work/school_stuffs/homeworks/STATS302/output_root C:/Users/gigggjjffd/Desktop/work/school_stuffs/homeworks/STATS302/raw.csv
    python data_processing.py build-train --axisless-root trajectories_axisless --raw-csv raw.csv --output-dir datasets
...
def build_cumulative_tables(match_id: str, frames: List[Dict[str, Any]], puuids: List[str],
    ...
    # If overview is present, override champ_map using puuid order => participantId mapping
    if overview_df is not None and puuids:
        ...
        for idx, puuid in enumerate(puuids, start=1):  # participantId = index in list (1-based)
...
def build_minute_dataset(timeline_dir: Path,
    ...
    for timeline_path in sorted(timeline_dir.glob('*.json')):
        ...
        if written_this_match > 0:
            ...
            # no valid minutes with images; optional cleanup
