...
class TabularOnlyClassifier(nn.Module):
    def __init__(self, num_feats: int, tab_embed_dim: int = 128, fusion_hidden: int = 256, dropout: float = 0.2):
        ...
        self.fusion = nn.Sequential(
            nn.Linear(tab_embed_dim, fusion_hidden), nn.ReLU(), nn.Dropout(dropout),
            nn.Linear(fusion_hidden, fusion_hidden//2), nn.ReLU(), nn.Dropout(dropout),
...
class TabularOnlyDataset(Dataset):
    ...
    def _read_table_to_features(self, csv_path: str) -> Tuple[np.ndarray, int]:
        ...
        cols_to_drop = ['champion_id', 'deaths']
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if auc > best_auc:
            ...
            print(f"Saved new best checkpoint without death and champion_id with AUC={auc:.4f}")

    df = pd.DataFrame(results)
    df.to_csv('training_results_no_id_death.csv', index=False)
    print("Saved training results to 'training_results_no_id_death.csv'")
